...
// 1. В одну строчку 3 задание из классной работы
// 2. Пункт номер 6 из классной работы* , проверьте свои силы, задание сложное
...
const getGenres: string[] = movies.map((movie: IMovie): IMovie["genre"] => movie.genre).flat();

function uniqueArrayOfGenres(arr: string[]) {
	let result: string[] = [];
...
// console.log(uniqueArrayOfGenres(getGenres));

// 2. Собрать в массив всех актеров всех фильмов (без повторения)

const getActors: string[] = movies.map((movie: IMovie): IMovie["actors"] => movie.actors).flat();

function uniqueArrayOfActors(arr: string[]) {
	let result: string[] = [];
...
// console.log(uniqueArrayOfActors(getActors));

// 3. Отсортировать фильмы по рейтингу по убыванию
const sortedMoviesByRating = (movies: Array<IMovie>) => {
	return movies.sort(function compare(a: IMovie,b: IMovie) {
...
// console.log(sortedMoviesByRating(movies));
